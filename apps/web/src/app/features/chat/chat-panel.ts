...
import { catchError, distinctUntilChanged, startWith, switchMap, takeWhile } from 'rxjs/operators';
import { toast } from 'ngx-sonner';
import type { ModelStatus, ChatMessage, SlashMention, SlashCommandType } from '@formunauts/shared';
...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, NgTemplateOutlet, NgIcon, Modal, TextInput],
  providers: [provideIcons({
    solarChatRoundBoldDuotone, solarFullScreenSquareBoldDuotone, solarShareBoldDuotone,
    solarFlagBoldDuotone, solarUserRoundedBoldDuotone, solarCardBoldDuotone,
    solarHashtagBoldDuotone, solarCalendarBoldDuotone, solarTagBoldDuotone,
  })],
...
export class ChatPanel implements OnInit, OnDestroy {
  ...
  readonly modelReady = computed(() => this.modelStatus()?.model_loaded ?? false);
  ...
  ngOnInit(): void {
    ...
    if (token) {
      this.#chatService.openSharedThread(token);
    }
  }
  ...
  #startPolling(): void {
    ...
    this.#pollSub = interval(2000).pipe(
      startWith(0),
      switchMap(() =>
        this.#chatService.getModelStatus().pipe(catchError(() => of(null))),
      ),
      takeWhile((s) => s === null || (!s.model_loaded && s.phase !== 'error'), true),
    ).subscribe((status) => {
      if (status) this.modelStatus.set(status);
    });
  }
